...
public class KeystoreService implements AbstractCrudService<SaveKeystoreRequestDto, SaveEntityResponseDto, KeystoreDto, KeystoreListDto>, CountService,
	...
	public SaveEntityResponseDto save(String model, MultipartFile file) {
		...
		if (EntityStatus.DISABLED == newEntity.getStatus()) {
			publishEvent(initMetaData(newEntity.getId()), EntityChangeEvent.EntityChangeType.KEYSTORE_DISABLED);
		}
	...
	public void toggleStatus(Long id, boolean enabled) {
		...
		publishEvent(initMetaData(entity.getId()), EntityChangeEvent.EntityChangeType.KEYSTORE_DISABLED);
	}
	...
	private void deleteFileById(Long id, Long userId, String fileName, boolean publishEvent) {
		...
		try {
			...
			if (publishEvent) {
				publishEvent(initMetaData(id), EntityChangeEvent.EntityChangeType.KEYSTORE_DELETED);
			}
		} catch (IOException e) {
	...
	private void processAlias(KeystoreEntity newEntity, KeystoreAliasDto alias) {
		if (AliasOperation.SAVE == alias.getOperation()) {
			...
			publishEvent(metadata, EntityChangeEvent.EntityChangeType.KEYSTORE_ALIAS_REMOVED);
		}
	}

	private void publishEvent(Map<String, Object> metadata, EntityChangeEvent.EntityChangeType entityChangeType) {
